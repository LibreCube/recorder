...
#endif
{
    ...
    void drawhud(fpsent *d, int w, int h)
    {
        if(d->state == CS_ALIVE)
        {
            ...
            {
                drawicon(flags[i].team==ctfteamflag(d->team) ? HICON_BLUE_FLAG : HICON_RED_FLAG, HICON_X + 3*HICON_STEP + (d->quadmillis ? HICON_SIZE + HICON_SPACE : 0), HICON_Y, 120, w, h);
                break;
            }
        }
    ...
    void rendergame()
    {
        loopv(flags)
        {
            ...
            //int type, int num, int fade, const vec &p, int color, float size, int radius, int gravity)
            //newparticle(vec(f.spawnloc.x, f.spawnloc.y, f.spawnloc.z+512), vec(f.spawnloc.x, f.spawnloc.y, f.spawnloc.z+512), 1, PART_GLOW, fs[i]?0xFFFFFF:0x000000, 10);

            vec lol; //occlusion check (neccessary as depth testing is turned off)
            if(raycubelos(vec(f.spawnloc.x, f.spawnloc.y, f.spawnloc.z+512), camera1->o, lol))
            ...
            vec lol2; //occlusion check (neccessary as depth testing is turned off)
            if(raycubelos(vec(f.spawnloc.x, f.spawnloc.y, f.spawnloc.z+10), camera1->o, lol2))
    ...
    void flagexplosion(int i, int team, const vec &loc)
    {
        ...
        particle_splash(PART_SPARK, 150, 300, loc, 0xFFFFFF, 0.1f);
    }
