...
#define super_name "SuperSauer 1.0.0"
#define super_sname "susa"
#define super_version "1.0.0"
#define super_url "http://supersauer.blogspot.com"
#ifdef WIN32
	#ifdef _WIN64
	  #define super_plattform "Win64"
	#else
	  #define super_plattform "Win32"
	#endif
#elif __APPLE__
	#define super_plattform "OSx"
#else
	#ifdef __x86_64
		#define super_plattform "Lin64"
	#else 
		#define super_plattform "Lin"
	#endif
#endif

#define super_irc "#supersauer"
...
inline char* klbuchstaben(char* inputstring) //convert string to small letters
{	
	int len = strlen(inputstring);
	char *convertstring = new char[len+1];
	//strcpy(convertstring, inputstring);
	for(int position = 0; position < len; position++){
		if(inputstring[position] >= 65 && inputstring[position] <= 90)
			convertstring[position] = inputstring[position]+32;
		else convertstring[position] = inputstring[position];
	}
	convertstring[len] = '\0';
