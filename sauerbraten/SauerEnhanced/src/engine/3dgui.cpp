...
struct gui : g3d_gui
{
    ...
    void start(int starttime, float initscale, int *tab, bool allowinput)
    {
        ...
        if(layoutpass)
        {
            scale.x = min(basescale*(totalmillis-starttime)/250.0f, basescale);
            scale.y = min(basescale*(totalmillis-starttime)/125.0f, basescale);
            scale.z = min(basescale*(totalmillis-starttime)/1.0f, basescale);
        }
