...
void drawdamagescreen(int w, int h)
{
    ...
    int slowerfade = removedscreen?1000:damagescreenfade;
    if(damageblendmillis - lastmillis < damagescreenfade || removedscreen)
...
string se_version = "v14.2 BETA";
string full_se_version = "SauerEnhanced v14.2 BETA";
...
void gl_drawhud(int w, int h)
{
    ...
    int abovehud = h*2.3f - FONTH, limitgui = h*3 - FONTH;
    ...
    if(!hidehud && !mainmenu)
    {
        if(!hidestats)
        {
            ...
            if(showfps == 2) draw_textf("fps %d+%d-%d", w*3-7*FONTH, h/32, curfps[0], curfps[1], curfps[2]);
            else if(showfps == 1) draw_textf("fps %d", w*3-5*FONTH, h/32, curfps[0]);
        ...
        if(hidestats || (!editmode && !showeditstats))
        {
            ...
            limitgui = min(h*3-FONTH, int(h*3*game::abovegameplayhud()));
        }
