...
void quit()                     // normal exit
{
    ...
    disconnect();
    localdisconnect();
    if(game::profileloaded) writecfg();
    cleanup();
    exit(EXIT_SUCCESS);
}
...
void checkfornewseversion()
{
    char buf[8];
    CURL *release = curl_easy_init();
    CURL *patch = curl_easy_init();
    curl_easy_setopt(release, CURLOPT_URL, "http://www.sauerenhanced.lua.pl/serelease.txt");
    if(!release)
    {
        conoutf("Could not connect to server/file doesn't exist");
        return;
    }
    FILE *file = fopen("serelease", "w");
    curl_easy_setopt(release, CURLOPT_WRITEDATA, file);
    curl_easy_perform(release);
    curl_easy_cleanup(release);
    fclose(file);

    stream *f = openfile("serelease", "r");
    f->getline(buf, sizeof(buf));
    sscanf(buf, "%d", &sev);
    f->close();
    if(sev > curseversion) showgui("NewVersion");
    else    //If there's no new release, check for patches for current release
    {
        curl_easy_setopt(patch, CURLOPT_URL, "http://www.sauerenhanced.lua.pl/sepatches.txt");
        if(!patch)
        {
            conoutf("Could not connect to server/file doesn't exist");
            return;
        }
        FILE *file = fopen("sepatches", "w");
        curl_easy_setopt(patch, CURLOPT_WRITEDATA, file);
        curl_easy_perform(patch);
        curl_easy_cleanup(patch);
        fclose(file);

        stream *f = openfile("sepatches", "r");
...
void getfile(int type)
{
    ...
    defformatstring(sefile)("SE_%d%s.zip", type==3?patchchoosen:sev, type==3?"_P":"");
    defformatstring(seurl)("http://sourceforge.net/projects/sauerenhanced/files/%s/download", sefile);
    string newsurl = "http://www.sauerenhanced.lua.pl/senews.txt";
    copystring(filename, type==1?"LastNews":sefile);

    curl_easy_setopt(handle, CURLOPT_URL, type==1?newsurl:seurl);

    FILE *file = fopen(filename, type==1?"w":"wb");
    ...
    curl_easy_cleanup(handle);
    fclose(file);
    if(type==1) execfile(filename);
    if(type==2) showgui("DownloadCompleted");
}
...
void renderprogress(float bar, const char *text, GLuint tex, bool background)   // also used during loading
{
    ...
    float bh = fh*20/52.0f,
          bx = fx + fw*17/511.0f, by = fy + fh*16/52.0f,
		  sw = fw*7/511.0f,
          mw = fw,
          ex = fx + max(mw*bar, fw*7/511.0f);
          float prg = bar;
    if(bar > 0)
    {
        ...
        glTexCoord2f(0, 0); glVertex2f(fx, fy);
        glTexCoord2f(prg, 0); glVertex2f(ex,    fy);
        glTexCoord2f(prg, 1); glVertex2f(ex,    fy+fh);
        glTexCoord2f(0, 1); glVertex2f(fx, fy+fh);
        glEnd();
    }
