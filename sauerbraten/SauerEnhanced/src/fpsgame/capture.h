...
#endif
{
    ...
    void rendertether(fpsent *d)
    {
        ...
        if(d->state==CS_ALIVE)
        {
            loopv(bases)
            {
                ...
                if(d==hudplayer()) particle_splash(PART_GLOW, 1, 1, pos, strcmp(d->team, player1->team) ? 0xFF0000 : 0x0000FF, 0.5f, 1, 0);
                if(oldbase < 0)
                {
                    particle_fireball(pos, 4.8f, PART_EXPLOSION_NO_GLARE, 250, strcmp(d->team, player1->team) ? 0x802020 : 0x2020FF, 4.8f);
                    particle_splash(PART_SPARK, 50, 250, pos, 0xFFFFFF, 0.1f);
                }
                d->lastbase = i;
            }
        }
    ...
    void rendergame()
    {
        ...
        loopv(bases)
        {
            ...
            vec above(b.ammopos);
            vec above_above(b.ammopos);
            above.z += FIREBALLRADIUS+1.0f;
            above_above.z += FIREBALLRADIUS+8.0f;
            particle_text(above, b.info, PART_TEXT, 1, tcolor, 2.0f);
            particle_text(above_above, b.name, PART_TEXT, 1, tcolor, 2.0f);
            if(mtype>=0)
            {
                above.z += 3.0f;
    ...
    void updatebase(int i, const char *owner, const char *enemy, int converted, int ammo)
    {
        ...
        if(strcmp(b.owner, owner)) particle_splash(PART_SPARK, 200, 250, b.ammopos, 0xFFFFFF, 0.1f);
        copystring(b.owner, owner);
        copystring(b.enemy, enemy);
        b.converted = converted;
