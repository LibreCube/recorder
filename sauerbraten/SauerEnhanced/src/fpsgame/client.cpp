...
namespace game
{
    ...
    VARP(sespawnfx, 0, 1, 1);

    void play(int n, vec p)
    {
        if(n==S_TELEPORT)
        {
        adddynlight(p, 1.15f*20, vec(0.5f, 0.5f, 2), 300, 100);
        particle_explodesplash(vec(p.x, p.y, p.z-2), 200, PART_TELE, 0xFFFFFF, 2, 500, 128);
        particle_explodesplash(vec(p.x, p.y, p.z-10), 200, PART_TELE, 0xFFFFFF, 2, 500, 128);
        }
        playsound(n, &p);
    }
    ...
    void parsemessages(int cn, fpsent *d, ucharbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            case SV_ITEMSPAWN:
            {
                ...
                if(sespawnfx)
                {
                    particle_fireball(entities::ents[i]->o, 10.1f, PART_EXPLOSION_NO_GLARE, 250, 0xFFFF00, 10.1f);
    ...
    void receivefile(uchar *data, int len)
    {
        ...
        switch(type)
        {
            case SV_SENDDEMO:
            {
                defformatstring(fname)("SauerEnhanced/profiles/%s/demos/%d.dmo", game::curprofile, lastmillis);
