...
namespace game
{
    ...
    void updatebouncers(int time)
    {
        loopv(bouncers)
        {
            ...
            if((bnc.bouncetype==BNC_GRENADE || bnc.bouncetype==BNC_GIBS)&& bnc.vel.magnitude() > 50.0f)
            {
                vec pos(bnc.o);
                pos.add(vec(bnc.offset).mul(bnc.offsetmillis/float(OFFSETMILLIS)));
                regular_particle_splash(PART_SMOKE, 1, 150, pos, bnc.bouncetype==BNC_GRENADE?0x00FFFF:0xFF0000, 1.f, 50, -20);
            }
            vec old(bnc.o);
            bool stopped = false;
    ...
    void damageeffect(int damage, fpsent *d, bool thirdperson)
    {
        ...
        if(blood && d!=hudplayer()) particle_splash(PART_BLOOD, 2, 150, p, 0x60FFFF, 10.f, 1, 0); //particle_splash(PART_STEAM, 5, 150, p, 0xFF0000, 10.f, 1, 0);
        if(thirdperson)
        {
    ...
    {
        if(!blood && damage > 30) return;
        vec from = d->abovehead();
        loopi(damage/10) spawnbouncer(from, vel, d, BNC_GIBS);
    }
    ...
    {
         owner->lastattackgun=gun;
        particle_splash(PART_SPARK, 200, 300, v, 0xB49B4B, 0.24f);
        playsound(gun!=GUN_GL ? S_RLHIT : S_FEXPLODE, &v);
        //particle_fireball(v, guns[gun].exprad/4, gun!=GUN_GL ? PART_EXPLOSION : PART_EXPLOSION_BLUE, gun!=GUN_GL ? -1 : int((guns[gun].exprad-4.0f)*5), gun!=GUN_GL ? 0xFF8080 : 0x80FFFF, 4.0f);
        //particle_splash(PART_SMOKE, 20, 2500, v, 0x222222, 15.0f, 50, 500, NULL, 1, false, 3);
        particle_splash(PART_SMOKE, 20, 200, v, 0x222222, 15.0f, 50, 500);
        //    particle_splash(PART_FLAME, 20, 30, v, 0xFFC864, 10.2f, 300, NULL, NULL, NULL, true);
        //    particle_splash(PART_FLAME, 20, 30, v, 0xFFC864, 10.2f, 300, NULL, NULL, NULL, true);
        //    particle_splash(PART_FLAME, 20, 30, v, 0xFFC864, 10.2f, 300, NULL, NULL, NULL, true);
           // particle_splash(PART_MUZZLE_FLASH3, 1, 100, v, 0xFFFFFF, 10.0f, 300, 500); //true, NULL, NULL, 4);
           particle_flare(v, v, 100, PART_MUZZLE_FLASH3, 0xFFC864, guns[gun].exprad);
            //particle_splash(PART_SPARK, 160, 30, v, 0xFFC864, 1.4f, 300, NULL, NULL, NULL, true);
            particle_fireball(v, guns[gun].exprad/2, PART_EXPLOSION, 100, 0xFF8080, 6.0f);
        adddynlight(v, 1.15f*guns[gun].exprad, vec(2, 1.5f, 1), 700, 100, 0, guns[gun].exprad/2, vec(1, 0.75f, 0.5f));
        ...
        loopv(projs)
        {
            ...
            if(!exploded)
            {
                ...
                else
                {
                    ...
                    else regular_particle_splash(PART_SMOKE, 2, 300, pos, 0xB49B4B, 2.4f, 50, -20);
                }
            }
            if(exploded)
            {
        ...
        switch(gun)
        {
            ...
            case GUN_SG:
            {
                ...
                if(muzzleflash && d->muzzle.x >= 0)
                    particle_flare(d->muzzle, d->muzzle, 200, PART_MUZZLE_FLASH3, 0xFF0000, 2.75f, d);
                loopi(guns[gun].rays)
                {
                    particle_splash(PART_SPARK, 20, 250, rays[i], 0xFF0000, 0.24f);
                    particle_flare(hudgunorigin(gun, from, rays[i], d), rays[i], 100, PART_STREAK, 0xFF0000, 0.28f);
            ...
            case GUN_PISTOL:
            {
                particle_splash(PART_SPARK, 200, 250, to, 0x00FF00, 0.24f);
                particle_flare(hudgunorigin(gun, from, to, d), to, 100, PART_STREAK, 0x00FF00, 0.28f);
                if(muzzleflash && d->muzzle.x >= 0)
                    particle_flare(d->muzzle, d->muzzle, gun==GUN_CG ? 100 : 200, PART_MUZZLE_FLASH1, 0x00FF00, gun==GUN_CG ? 2.25f : 1.25f, d);
            ...
            case GUN_RL:
                if(muzzleflash && d->muzzle.x >= 0)
                    particle_flare(d->muzzle, d->muzzle, 250, PART_MUZZLE_FLASH2, 0xB49B4B, 3.0f, d);
            case GUN_FIREBALL:
            case GUN_ICEBALL:
            case GUN_SLIMEBALL:
            ...
            case GUN_GL:
            {
                ...
                up.z += dist/8;
                if(muzzleflash && d->muzzle.x >= 0)
                    particle_flare(d->muzzle, d->muzzle, 200, PART_MUZZLE_FLASH2, 0x00FFFF, 1.5f, d);
            ...
            case GUN_RIFLE:
                //particle_splash(PART_SMOKE, 3, 800, to, 0xFF00FF, 2.f, 1, 0);
                vec p;
                p.x = p.y = p.z = 0.f;
                vecfromyawpitch(d->yaw, d->pitch, 1, 0, p);
                float barrier = 1000.f; //raycube(p.owner->o, p.to, guns[p.gun].range, RAY_CLIPMAT|RAY_ALPHAPOLY);
                //float barrier = dspeed ? raycube(o, dir, dspeed, RAY_CLIPMATE, RAY_ALPHAPOLY) : 0;
                p.mul(barrier).add(to);
                loopi(3)particle_flare(d->muzzle.x>0?d->muzzle:d->o, p, 1000, PART_STREAK, 0x0000FF, .8f);
                //particle_trail(PART_SMOKE, 700, hudgunorigin(gun, from, to, d), to, 0xFF00FF, 0.6f);
                if(muzzleflash && d->muzzle.x >= 0)
                    particle_flare(d->muzzle, d->muzzle, 150, PART_MUZZLE_FLASH3, 0x0000FF, 1.25f, d);
