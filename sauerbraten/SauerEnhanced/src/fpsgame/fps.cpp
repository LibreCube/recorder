...
namespace game
{
    ...
    void retoserver(char *text) { if(!seautosay) return; else conoutf(CON_CHAT, "%s:\f0 %s", colorname(player1), text); addmsg(SV_TEXT, "rcs", player1, text); }
    void resayteam(char *text) { if(!seautosay) return; else conoutf(CON_TEAMCHAT, "%s:\f1 %s", colorname(player1), text); addmsg(SV_SAYTEAM, "rcs", player1, text); }
    ...
    void killed(fpsent *d, fpsent *actor)
    {
        ...
        {
            if(d==player1) conoutf(contype, "\f6You died!");
            else conoutf(contype, "\f2%s died", dname);
        }
        else if(isteam(d->team, actor->team))
        {
            if(actor==player1) {conoutf(contype, "\f3you fragged a teammate (%s)", dname); defformatstring(sry)("Sorry, %s!", dname); resayteam(sry);
                ffrag = 1;
                when = totalmillis;
            ...
            else conoutf(contype, "\f3%s fragged a teammate (%s)", aname, dname);
        ...
        defformatstring(who4)("\f2You got killed by %s %s", who3, ismate?"\f3your TEAMMATE!":"!");
        copystring(who5, who4);
        defformatstring(who2)("You killed %s %s", who, ismate2?"\f3your TEAMMATE!":"!");
        copystring(who6, who2);
        deathstate(d);
		ai::killed(d, actor);
    }

    SVAR(seendmsg, "Good game!");
    ...
    int selectcrosshair(float &r, float &g, float &b, int &w, int &h)
    {
        ...
        else if(teamcrosshair)
        {
            ...
            if(o && o->type==ENT_PLAYER && isteam(((fpsent *)o)->team, d->team))
            {
                crosshair = 1;
                r = g = 0;
                if(seteamnames) draw_text(((fpsent *)o)->name, w/2, h/3, 64, 64, 255);
            }
        }
        ...
        if(ffrag && totalmillis-when<=4000)
        {
            ...
            glPushMatrix();
            glScalef(h/1800.0f, h/1800.0f, 1);
            draw_textf(who6, w*1800/h - tw - pw*14, 1650 - th - ph/2, who);
            glPopMatrix();
        }
        if(ffrag2 && totalmillis-when2<=4000)
        {
            ...
            glPushMatrix();
            glScalef(h/1800.0f, h/1800.0f, 1);
            draw_textf(who5, w*1800/h - tw - pw*14, 1600 - th - ph/2, who);
            glPopMatrix();
        }
