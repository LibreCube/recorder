...
void writesefile()
{
	defformatstring(file)("ents/%s.se", game::getclientmap());
	stream *f = openfile(file, "w");
	if(!f)
	{
		conoutf("SE file does not exist/could not be created");
		return;
	}
	...
	loopv(entities::getents())
	{
		...
		if((tmp.type == ET_SOUND && tmp.attr5 == 1) || (tmp.type == ET_PARTICLES && tmp.attr1 >= 77))
		{
			//f->write(&tmp, sizeof(entity));
			...
			if(sefiledbg) conoutf("NUM: %i | TYPE: %s | ATTRS: %i %i %i %i %i | POS: %f %f %f", numents+1, tmp.type==5?"particles":"sound", tmp.attr1, tmp.attr2, tmp.attr3, tmp.attr4, tmp.attr5, tmp.o.x, tmp.o.y, tmp.o.z);
			numents++;
		}
	}
...
{
	if(!loadse) return;
	execfile("SauerEnhanced/ambience.cfg");
	...
	execfile("SauerEnhanced/ambience2.cfg");
	execfile("SauerEnhanced/ambience3_doomish.cfg");
	execfile("SauerEnhanced/ambience4.cfg");
	defformatstring(file)("SauerEnhanced/ents/%s.se", game::getclientmap());
	...
	if(!f)
	{
        conoutf("SE file does not exist - no additional map effects.");
        return;
    }

	conoutf("Found SE file for %s. Loaded additional map effects.", game::getclientmap());
	vector<extentity *> &ents = entities::getents();
        ...
        while(f->getline(buf, sizeof(buf)))
        {
            ...
            if(sefiledbg) conoutf("NUM: %i | TYPE: %s | ATTRS: %i %i %i %i %i | POS: %f %f %f", entnum, type==5?"particles":"sound", a1, a2, a3, a4, a5, x, y, z);
